...
const nextVersion = ({ currVer, date, increment, style=STYLE_AUTO }) => {
  if (increment !== undefined && !validIncrements.includes(increment))
  ...
  if (currType === null && increment === 'prerelease')
    throw createError.BadRequest('Cannot increment prerelease for non-prerelease versions.')
  if (currType !== null && !(increment === 'prerelease' || increment === 'pretype'))
